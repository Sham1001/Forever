...
<div key={index} className='flex justify-between  p-4 border-t border-b text-gray-300 grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
               ...
               {/* <div className='flex items-center justify-center'> */}
                ...
                <input className='text-gray-900 border max-w-10 sm:max-w-20  sm:px-2 ' type="number" min={1} onChange={(e)=> e.target.value === '' || e.target.value === '0' ? null : updateCartInfo(items.id,items.size,Number(e.target.value))} defaultValue={items.quantity} />
               <img onClick={()=>updateCartInfo(items.id,items.size,0)} className='w-5 sm:w-7 cursor-pointer' src={assets.bin_icon} alt='Delete_button' />
              </div>
              // </div>
     ...
     <div className='sm:w-[20%] w-[120px] sm:h-10 h-[50px]  bg-black text-white text-center justify-center flex mt-5 relative sm:left-[80%] left-[70%] mb-40'>
