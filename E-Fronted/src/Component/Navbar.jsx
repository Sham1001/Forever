...
const Navbar = () => {
  ...
  const { showSearch, setShowSearch, token, setToken, getCartCount, setCartItems, navigate } = useContext(ShopContext)
      ...
      <nav className='flex justify-between py-6 items-center font-medium '>
        ...
        <ul className='hidden sm:flex gap-5 text-sm text-gray-700  '>
          ...
          <div>
            <button
              onClick={() =>
                window.open(import.meta.env.VITE_ADMIN_PANEL, "_blank", "noopener,noreferrer")
              }
              className="border rounded-2xl cursor-pointer px-2 font-medium text-center"
            >
              Admin Panel
            </button>
        ...
        <div className='flex items-center gap-6 '>
          ...
          <div className='group relative '>
            <img onClick={() => { token ? null : navigate('/login') }} className='w-5' src={assets.profile_icon} alt="" />

            {
              token &&

              <div className='group-hover:block hidden right-0 pt-4 absolute dropdown-menu'>
                <div className='flex flex-col gap-2 py-3 w-36 pl-5 bg-slate-100 text-gray-500 rounded'>
                  <p className='cursor-pointer hover:text-black'>My Profile</p>
                  <p onClick={() => navigate('/order')} className='cursor-pointer hover:text-black'>My Order</p>
        ...
        <div className={`absolute top-0 right-0 bottom-0 ${active ? 'w-full' : 'w-0'} overflow-hidden bg-white transition-all`}>
          <div className='flex flex-col  text-center'>
            ...
            <div className='space-y-3 flex flex-col '>
              ...
              <NavLink onClick={() => setActive(false)} to={import.meta.env.VITE_ADMIN_PANEL} className='py-2 cursor-pointer border'>ADMIN PANEL</NavLink>
            </div>
          </div>
